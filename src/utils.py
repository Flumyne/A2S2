...
class Normalizer(nn.Module):
    ...
    """
    def __init__(self, tensor=None, num_features=1, device='cpu'):
        super().__init__()

        if tensor is not None:
            mean = torch.mean(tensor, dim=0).detach().to(device)
            std = torch.std(tensor, dim=0).detach().to(device)
            std[std < 1e-6] = 1.0
        else:
...
def visualize_loss(loss_history, loss_energy_history, loss_bc_left_history, loss_U_history, loss_W_ext_history):
    ...
    plt.tight_layout()
    plt.savefig("Residual_A2S2_V0_298.png", dpi=150, bbox_inches='tight')
