...
class FourierEmbedding(nn.Module):
    ...
    """
    def __init__(self, in_features, n_freq=4):
        super().__init__()
...
class NeuralNet(nn.Module):
    ...
    """
    def __init__(self, normalizer_spatial=None, normalizer_param=None, hidden_dim=32, num_layers=5, use_fourier=True):
        super(NeuralNet, self).__init__()

        self.normalizer_s = normalizer_spatial if normalizer_spatial else Normalizer(num_features=2)
        self.normalizer_p = normalizer_param if normalizer_param else Normalizer(num_features=3)
        ...
        if use_fourier:
            self.embedding = FourierEmbedding(input_dim_s, n_freq=4)
